...
async function testPortfolioService() {
  ...
  try {
    ...
    const portfolio = await portfolioService.getPortfolioForUser(testAddress, ['ETHEREUM_MAINNET']);
    
    console.log(`✅ Successfully fetched portfolio with ${portfolio.length} tokens`);
